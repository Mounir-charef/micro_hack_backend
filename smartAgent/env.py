...
class Env:
    options = [None, " ", "\n"] + list(
        "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMOPQRSTUVWXYZ/*-\\_.,?!$£%@()"
    )
    ...
    def __init__(self) -> None:
        ...
        self.docs = [
            i
            for i in Path("E:\\mchack\\findit2\\train\\").iterdir()
    ...
    def get_doc(self):
        ...
        self.false_doc = list(
            "".join(
                [
                    detection[1]
                    for detection in self.reader.readtext(
                        "train\\" + self.doc.stem + ".png"
                    )
                ]
            )
        )
